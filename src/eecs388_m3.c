...
{
    int ang = angle;
    unsigned int x = getServoCycle(ang);
    breakup(x, bufWrite[3], bufWrite[4]);
    metal_i2c_transfer(i2c, PCA9685_LED1_ON_L  , bufWrite,3 , bufWrite,4);
...
void stopMotor()
{
    unsigned int x = 280;
    breakup(x, bufWrite[3], bufWrite[4]);
    metal_i2c_transfer(i2c, PCA9685_LED0_ON_L  , bufWrite,3 , bufWrite,4 );
}

void driveForward(uint8_t speedFlag)
{
    uint8_t speed = speedFlag;
    breakup(speed, bufWrite[3], bufWrite[4]);
    metal_i2c_transfer(i2c,PCA9685_LED0_ON_L , bufWrite,3 , bufWrite,4 );
}
